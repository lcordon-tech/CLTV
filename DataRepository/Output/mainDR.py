...
Versión MULTI-PAÍS con tipo de cambio dinámico
"""
...
def run_pipeline():
    ...
    try:
        ...
        q_engine = QueryEngine(user=USER, password=PASSWORD, host=HOST, db=DB)
        ...
        # ⭐ MetricsCalculator AHORA recibe country_context y fx_engine
        ...
        # Modificar DataLoader para que use las hojas correctas del país
        # Nota: DataLoader necesita ser modificado para recibir country_context
        # Por ahora, asumimos que DataLoader ya fue modificado o que los archivos
        # tienen la estructura esperada con hojas por país
        
        datasets = loader.load_all_sources(
            PATH_SOIS, PATH_SUPUESTOS, PATH_CATALOGO,
            start_date=start_date, 
            end_date=end_date,
            country_context=country_context  # ← NUEVO parámetro
        )
        ...
        if country_context.cohort_start_year > 2021:
            # Convertir cohort a año para filtrar
