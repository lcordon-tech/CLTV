...
class CohortSupuestosManager:
    ...
    def __init__(self, supuestos_path: str, country_code: str = "GT"):
        """
        Args:
    ...
    def _detect_mode(self) -> str:
        """
        Detecta el modo de operación según las hojas del Excel.
        
        Returns:
            'country_suffix': busca hojas como 1PGT, 1PCR (sufijo por país)
            'legacy': busca hojas como 1P, 3P (sin sufijo)
        """
        try:
            xl = pd.ExcelFile(self.supuestos_path)
            sheet_names = xl.sheet_names
            
            # Verificar si existen hojas con sufijo de país
            test_sheet = f"1P{self.country_code}"
            if test_sheet in sheet_names:
                print(f"📋 Modo multi-país detectado: usando hojas con sufijo '{self.country_code}'")
